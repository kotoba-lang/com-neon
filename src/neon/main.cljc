(ns neon.main
  "Kotodama WASM entrypoint for the Neon clean-room actor (L4) — Clojure port.
  ...
  facts (`neon.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Service"    :plural "services"    :id-prefix "neon_ser"
    :fields [:name :runtime :region :status]   :required [:name :runtime]
    :coerce {}                       :refs {}}
   {:entity "Deployment" :plural "deployments" :id-prefix "neon_dep"
    :fields [:serviceId :version :status :deployedAt] :required [:version :status]
    :coerce {}                       :refs {:serviceId "Service"}}
   {:entity "Secret"     :plural "secrets"     :id-prefix "neon_sec"
    :fields [:name :scope :rotatedAt]          :required [:name :scope]
    :coerce {}                       :refs {}}
   {:entity "ApiProxy"   :plural "apiproxies"  :id-prefix "neon_api"
    :fields [:name :basePath :target :active]  :required [:name :basePath]
    :coerce {:active :bool}          :refs {}}
   {:entity "Database"   :plural "databases"   :id-prefix "neon_dat"
    :fields [:name :engine :region :sizeGb]   :required [:name :engine]
    :coerce {:sizeGb :int}           :refs {}}
   {:entity "Function"   :plural "functions"   :id-prefix "neon_fun"
    :fields [:serviceId :name :memoryMb :timeoutMs] :required [:name :memoryMb]
    :coerce {:memoryMb :int :timeoutMs :int}   :refs {:serviceId "Service"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"neon.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "neon-compat" :tier tier :entities entities} 200])
